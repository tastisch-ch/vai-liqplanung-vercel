    ...
    <DayPicker
      ...
      components={{
        ...
        Caption: ({ ...captionProps }) => {
          ...
          const displayIndex = displayMonths.findIndex((m) => isSameMonth((captionProps as any).displayMonth, m))
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1">
                {enableYearNavigation && !hidePreviousButton && (
                  <NavigationButton disabled={disableNavigation || !previousMonth || (fromDate && addYears(currentMonth, -1).getTime() < fromDate.getTime())} aria-label="Go to previous year" onClick={goToPreviousYear} icon={RiArrowLeftDoubleLine} />
                )}
                {!hidePreviousButton && (
                  <NavigationButton disabled={disableNavigation || !previousMonth} aria-label="Go to previous month" onClick={() => previousMonth && goToMonth(previousMonth)} icon={RiArrowLeftSLine} />
                )}
              </div>
              <div role="presentation" aria-live="polite" className="text-sm font-medium capitalize tabular-nums text-gray-900 dark:text-gray-50">
                {format((captionProps as any).displayMonth, 'LLLL yyy', { locale })}
              </div>
              <div className="flex items-center gap-1">
                {!hideNextButton && (
                  <NavigationButton disabled={disableNavigation || !nextMonth} aria-label="Go to next month" onClick={() => nextMonth && goToMonth(nextMonth)} icon={RiArrowRightSLine} />
                )}
                {enableYearNavigation && !hideNextButton && (
                  <NavigationButton disabled={disableNavigation || !nextMonth || (toDate && addYears(currentMonth, 1).getTime() > toDate.getTime())} aria-label="Go to next year" onClick={goToNextYear} icon={RiArrowRightDoubleLine} />
                )}
              </div>
            </div>
          )
        ...
        Day: ({ date, displayMonth }: DayProps) => {
          ...
          const { activeModifiers, buttonProps, divProps, isButton, isHidden } = useDayRender(date, displayMonth, buttonRef as React.RefObject<HTMLButtonElement>)
          ...
          return (
            <button ref={buttonRef as React.RefObject<HTMLButtonElement>} {...buttonPropsRest} type="button" className={cx('relative', buttonClassName)}>
              {buttonChildren}
              {today && (
                <span
                  className={cx('absolute inset-x-1/2 bottom-1.5 h-0.5 w-4 -translate-x-1/2 rounded-[2px] ' +
                    (!selected ? 'bg-blue-500 dark:bg-blue-500 ' : '') +
                    (selected && range_middle ? 'bg-gray-400 dark:bg-gray-600 ' : '') +
                    (selected && !range_middle ? 'bg-white dark:bg-gray-950 ' : '') +
                    (disabled ? 'bg-gray-400 text-gray-400 dark:bg-gray-400 dark:text-gray-600 ' : ''))}
                />
              )}
            </button>
          )
